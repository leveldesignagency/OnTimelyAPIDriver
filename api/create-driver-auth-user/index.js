  ...
  try {
    ...
    try {
      ...
      if (RESEND_API_KEY) {
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
              ...
              <p>Hello ${fullName},</p>
              <p>Your driver account has been created. Use the credentials below to login:</p>
              <div style="background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin: 20px 0;">
                <p style="margin: 0 0 10px 0;"><strong>Email:</strong> ${email}</p>
                <p style="margin: 0;"><strong>Password:</strong> <code style="background-color: #fff; padding: 4px 8px; border-radius: 4px; font-family: monospace;">${password}</code></p>
              </div>
              <p style="margin: 30px 0;">
                <a href="${driverPortalUrl}" 
        ...
        console.log('RESEND_API_KEY not set - skipping email send. Credentials:', { email, password });
      }
    } catch (emailErr) {
